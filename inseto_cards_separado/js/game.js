...
const CARDS={
 ...
 casca:{name:'Casca',cost:3,type:'effect',equip:true,emoji:'🥚',cond:'Equipável',ability:'Ignora o próximo debuff recebido, incluindo virar Larva.'},
 propolis:{name:'Própolis',cost:3,type:'effect',equip:true,emoji:'🍯',cond:'Equipável',ability:'Remove veneno da carta equipada.'},
...
const DECK_KEYS=Object.keys(CARDS); // 41 cartas disponíveis, uma cópia de cada carta
// ================================================================
// 2. ESTADO DA PARTIDA
...
function card(key,owner){let c=CARDS[key];return{id:Math.random().toString(36).slice(2),key,owner,atk:c.atk??0,hp:c.hp??0,maxHp:c.hp??0,baseAtk:c.atk??0,baseHp:c.hp??0,damage:0,buffs:[],poison:0,poisonTurns:0,root:0,skipAttack:0,reload:0,protectedOnce:key==='louva',barataUsed:false,mel:false,casca:true,customAbility:null,copiedKey:null,debuffNext:false};}
...
function drawCost(pi){let p=state.players[pi];if(p.leaves<3||!state.deck.length)return false;p.leaves-=3;let c=drawRaw(pi);if(c)log(`${p.name} comprou ${CARDS[c.key].name} por 3 folhas.`);return !!c}
...
function winCheck(){if(allInsectsGone(0)||allInsectsGone(1)){state.over=true;let winner=allInsectsGone(1)?0:1;showModal(`<div class="win">${winner===0?'VOCÊ VENCEU!':'BOT VENCEU!'}</div><p>Condição: todas as cartas inseto do adversário foram derrotadas. Cartas de efeito restantes não impedem a vitória.</p>`);return true}return false}
function startTurn(pi){if(state.over)return;state.turn=pi;let p=state.players[pi];p.moves=1;p.std=1;if(state.skip[pi]){state.skip[pi]=false;p.moves=0;p.std=0;log(`${p.name} perdeu este turno.`);if(pi===1){setTimeout(()=>endTurn(1),700)}return}if(state.round>1){p.leaves=Math.min(15,p.leaves+1)};for(let x of [p.front,...p.bank])if(x){if(x.root>0)x.root--;if(x.skipAttack>0)x.skipAttack--;if(x.reload>0)x.reload--;if(x.poisonTurns>0){x.hp-=1;x.poisonTurns--;log(`${CARDS[x.key].name} sofreu 1 de Veneno.`)}else x.poison=0;if(x.lupa>0){x.hp-=2;x.lupa--;log(`${CARDS[x.key].name} sofreu 2 dano da Lupa.`);if(x.hp<=0)defeat(p,x,1-pi)}};if(p.adubo>0)p.adubo--;if(p.antiSteal>0)p.antiSteal--;if(pi===0)msg('Seu turno. Escolha uma ação.');else botTurn()}
function endTurn(pi){if(state.over)return;if(pi===0){state.round++;startTurn(1)}else{state.round++;startTurn(0)};render();winCheck()}
// ---------------------------------------------------------------
...
// ---------------------------------------------------------------
function summonFromHand(pi,idx,zone,slot){let p=state.players[pi],c=p.hand[idx];if(!c)return false;let d=CARDS[c.key];if(d.type==='effect')return playEffect(pi,idx);if(p.leaves<d.cost){msg('Folhas insuficientes.');return false}if(zone==='front'&&p.front)return false;if(zone==='bank'&&p.bank[slot])return false;p.leaves-=d.cost;p.hand.splice(idx,1);c.owner=pi;if(zone==='front')p.front=c;else p.bank[slot]=c;p.moves--;log(`${p.name} invocou ${d.name} no ${zone==='front'?'Fronte':'Banco'}.`);onSummon(pi,c);triggerCopied(p.front&&p.front!==c?p.front:null,'summon',pi,c);triggerCopied(c,'summon',pi,c);render();winCheck();return true}
...
// ---------------------------------------------------------------
function moveCard(pi,from,slot){let p=state.players[pi];if(p.moves<=0)return false;if(from==='front'){if(!p.front)return false;let dest=p.bank.findIndex(x=>!x);if(dest<0)return false;let c=p.front;p.front=null;p.bank[dest]=c;c.root=0;p.moves--;log(`${p.name} moveu ${CARDS[c.key].name} para o Banco.`)}else{let c=p.bank[slot];if(!c||p.front)return false;p.bank[slot]=null;p.front=c;c.root=0;p.moves--;log(`${p.name} moveu ${CARDS[c.key].name} para o Fronte.`)}render();return true}
function returnToHand(pi,zone,slot){let p=state.players[pi],c=zone==='front'?p.front:p.bank[slot];if(!c||c.damage>0)return false;if(p.moves<=0)return false;if(zone==='front')p.front=null;else p.bank[slot]=null;p.hand.push(c);p.moves--;log(`${p.name} retornou ${CARDS[c.key].name} para a mão.`);render();return true}
...
// ---------------------------------------------------------------
function attack(pi,a,t){let p=state.players[pi],o=state.players[1-pi];if(p.std<=0)return false;if(a.root>0||a.skipAttack>0||a.reload>0)return false;if(!t)return false;let d=CARDS[a.key];p.std--;let damage=Math.max(0,a.atk);if(a.key==='besouro'&&a.hp<=Math.ceil(a.maxHp/2))damage*=2;if(a.key==='cupim')clearBuffs(t);if(a.key==='gafanhoto')steal(pi,1);if(a.key==='grilo'){p.leaves=Math.min(15,p.leaves+1)}if(a.key==='escorpiao'){t.poisonTurns=Math.max(t.poisonTurns,2);t.poison=1}if(a.key==='aranha'&&!isImmuneRoot(t)){if(t.casca){t.casca=false;log(`${CARDS[t.key].name} usou a Casca para ignorar a Teia.`)}else t.root=Math.max(t.root,1)}if(a.key==='centopeia'&&t.hp<=(t.maxHp/2)&&!isImmuneRoot(t)){if(t.casca){t.casca=false;log(`${CARDS[t.key].name} usou a Casca para ignorar a Centopeia.`)}else t.root=Math.max(t.root,2)}if(a.key==='meganeura')a.reload=1;let dealt=damage;if(a.poisonTurns>0&&a.poison>0)dealt+=1;if(a.key==='escaravelho'){};if(t.key==='escaravelho')dealt=Math.max(0,dealt-1);if(t.key==='bichoPau'){dealt=0}if(t.key==='louva'&&t.protectedOnce){dealt=0;t.protectedOnce=false;log('Louva-Deus anulou o primeiro ataque recebido.')}t.hp-=dealt;log(`${p.name}: ${d.name} causou ${dealt} dano em ${CARDS[t.key].name}.`);if(t.hp>0&&insect(t)&&! (t.key==='bichoPau')){let back=Math.max(0,t.atk);if(a.key==='escaravelho')back=Math.max(0,back);if(a.key==='bichoPau')back=0;if(a.key==='escorpi')back=0;if(a.key==='louva')back=Math.max(0,back);if(t.key==='besouro'&&t.hp<=Math.ceil(t.maxHp/2)){}if(a.key==='centopeia'){}if(a.key==='meganeura'){};a.hp-=back;log(`${CARDS[t.key].name} devolveu ${back} dano.`)};if(t.hp<=0){triggerCopied(a,'defeat',pi,t);defeat(o,t,pi)}else triggerCopied(a,'attack',pi,t);if(a.hp<=0)defeat(p,a,1-pi);render();winCheck();return true}
...
function defeat(owner,c,killer){let p=owner,idx=-1;if(p.front===c){p.front=null}else idx=p.bank.indexOf(c);if(idx>=0)p.bank[idx]=null;let d=CARDS[c.key];log(`${p.name}: ${d.name} foi derrotado.`);if(c.key==='barata'&&!c.barataUsed){c.barataUsed=true;c.atk*=2;c.hp*=2;c.maxHp*=2;p.hand.push(c);log('Barata voltou para a mão com atributos dobrados.')}else{state.grave.push(c);if(c.key==='cigarra'){let defeatedPi=p===state.players[0]?0:1;state.skip[1-defeatedPi]=true;}if(c.key==='ovos')drawFree(p===state.players[0]?0:1,2);if(c.key==='formigaVermelha'){let kpi=killer;let killerCard=kpi.front;if(killerCard){killerCard.atk+=1;killerCard.maxHp+=1;killerCard.hp+=1}}if(c.key==='joaninha'){for(let x of [p.front,...p.bank])if(x)x.hp=Math.min(x.maxHp,x.hp+1)}let pi=p===state.players[0]?0:1;let ally=state.players[pi];if(ally.bank.some(x=>x&&x.key==='rainhaAbelha'))drawFree(pi,1);if(ally.front&&ally.front.key==='joaninha'){for(let x of [ally.front,...ally.bank])if(x&&x!==c)x.hp=Math.min(x.maxHp,x.hp+1)}if(ally.front&&ally.front.key==='varejeira'&&killer===pi){let s=ally.bank.findIndex(x=>!x);if(s>=0)ally.bank[s]=card('larvas',pi)}if(killer===pi&&ally.front&&ally.front.key==='vespa')ally.front.debuffNext=true;if(killer===pi&&ally.front&&ally.front.key==='maribondo'){ally.front.customAbility=d.ability;ally.front.copiedKey=c.key;log(`Maribondo copiou a habilidade de ${d.name}.`);}if(killer===pi&&ally.front&&ally.front.key==='percevejo')ally.front.hp=ally.front.maxHp}}
...
function playEffect(pi,idx){let p=state.players[pi],c=p.hand[idx],d=CARDS[c.key];if(p.std<=0||p.leaves<d.cost)return false;if(d.equip){if(!p.front&&!p.bank.some(Boolean)){msg('Não há inseto para equipar.');return false}state.targetMode={type:'equip',pi,idx};msg('Clique no seu inseto para equipar.');return false}if(['inseticida','lupa','teia'].includes(c.key)){state.targetMode={type:'effectTarget',pi,idx};msg('Clique no alvo inimigo.');return false}p.leaves-=d.cost;p.hand.splice(idx,1);resolveEffect(pi,c);p.std--;render();return true}
function resolveEffect(pi,c){let p=state.players[pi],o=state.players[1-pi];switch(c.key){case'adubo':p.adubo=4;break;case'inseticida':{let t=o.bank.find(Boolean);if(t){t.hp-=1;if(t.hp<=0)defeat(o,t,pi)}break}case'lupa':{let t=o.front||o.bank.find(Boolean);if(t){t.hp-=2;if(t.hp<=0)defeat(o,t,pi);log('Lupa aplicou 2 de dano imediato; o efeito contínuo fica representado pelo estado curto da partida.')}break}case'teia':{let t=o.front||o.bank.find(Boolean);if(t&&!isImmuneRoot(t))t.skipAttack=1;break}case'formigueiro':p.antiSteal=3;break}}
function equip(pi,idx,target){let p=state.players[pi],c=p.hand[idx],d=CARDS[c.key];if(!c||p.leaves<d.cost)return false;p.leaves-=d.cost;p.hand.splice(idx,1);if(c.key==='mel'){target.atk+=1;target.maxHp+=1;target.hp+=1;target.mel=true}if(c.key==='casulo'){target.maxHp+=2;target.hp+=2}if(c.key==='veneno'){target.poison=1;target.poisonTurns=2}if(c.key==='casca')target.casca=true;if(c.key==='propolis'){target.poison=0;target.poisonTurns=0}p.std--;state.targetMode=null;log(`${p.name} equipou ${d.name} em ${CARDS[target.key].name}.`);render();return true}
...
function secondMove(){if(!validMove(0)||state.players[0].std<=0||state.players[0].moves<=0)return;state.players[0].std--;state.players[0].moves++;state.selectedField=null;log('Você abriu mão da ação padrão para obter um segundo movimento.');render()}
function harvest(pi){if(!validMove(pi)||state.players[pi].std<=0)return;let p=state.players[pi],gain=p.adubo>0?2:1;p.leaves=Math.min(15,p.leaves+gain);p.std--;log(`${p.name} colheu ${gain} folha(s).`);render()}
// ================================================================
// BOT
...
function botTurn(){setTimeout(()=>{if(state.over)return;let p=state.players[1],o=state.players[0];if(p.leaves<8&&p.std)harvest(1);let x=aiBestCard();if(x&&p.moves){let zone=!p.front?'front':p.bank.findIndex(v=>!v)>=0?'bank':null;if(zone==='front')summonFromHand(1,x.i,'front',0);else if(zone==='bank')summonFromHand(1,x.i,'bank',p.bank.findIndex(v=>!v))}if(p.front&&p.std){let targets=o.front?[o.front]:o.bank.filter(Boolean);if(targets.length){let t=targets.sort((a,b)=>a.hp-b.hp)[0];attack(1,p.front,t)}}if(p.std&&p.leaves>=3&&state.deck.length){drawCost(1);p.std--}if(p.std)harvest(1);if(!state.over)endTurn(1)},850)}
// ================================================================
// DRAG & DROP
// Cartas do jogador podem ser arrastadas para os slots válidos.
// ================================================================
function prepareDropTargets(c){if(state.turn!==0||state.over)return;document.querySelectorAll('.slot').forEach(s=>{let id=s.id;if(['pf0','pb0','pb1','pb2'].includes(id)){s.classList.add('drop-ready')}})}
function clearDropTargets(){document.querySelectorAll('.slot').forEach(s=>s.classList.remove('drop-ready','drag-over'))}
...
function handleDropOnSlot(slotEl,e){e.preventDefault();slotEl.classList.remove('drag-over');let id=e.dataTransfer.getData('text/plain');let found=findPlayerCardById(id);if(!found||state.turn!==0||state.over)return;let target=slotEl.id;let zone=target==='pf0'?'front':'bank';let slot=zone==='bank'?Number(target.slice(-1)):0;let p=state.players[0];let destOccupied=zone==='front'?!!p.front:!!p.bank[slot];if(destOccupied){msg('Esse slot já está ocupado.');return}
  ...
  else if(found.zone==='bank' && zone==='front'){if(state.players[0].front){msg('O Fronte já está ocupado.');return}let c=p.bank[found.slot];p.bank[found.slot]=null;p.front=c;c.root=0;p.moves--;log(`${p.name} moveu ${CARDS[c.key].name} para o Fronte.`);state.selectedField=null;render();}
  else msg('Arraste a carta para outro espaço válido.');
  clearDropTargets();render();winCheck();}
function installDropTargets(){document.querySelectorAll('#pf0,#pb0,#pb1,#pb2').forEach(slot=>{slot.addEventListener('dragover',e=>{if(!e.dataTransfer.types.includes('text/plain'))return;e.preventDefault();if(state.turn===0&&!state.over)slot.classList.add('drag-over')});slot.addEventListener('dragleave',()=>slot.classList.remove('drag-over'));slot.addEventListener('drop',e=>handleDropOnSlot(slot,e));});}
// ================================================================
// RENDERIZAÇÃO
...
function renderSlot(id,c,enemy,front){let s=document.getElementById(id);s.innerHTML='';if(!c){s.textContent=front?'FRONTE':'BANCO';return}let el=makeCard(c,enemy);if(front)el.classList.add('fronte-card');s.appendChild(el);el.onclick=()=>clickCard(enemy?1:0,front?'front':'bank',front?0:Number(id.slice(-1)));if(!enemy)el.title='Clique para mover';}
...
installDropTargets();render();log('Pronto. Clique em INICIAR PARTIDA.');
