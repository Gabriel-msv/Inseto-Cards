...
// ======================================================================
function buildGate() {
  if (document.getElementById('mpGate')) return;
  ...
  playerNameInput.value = document.getElementById('startName')?.value?.trim() || getSavedName();
  ...
  document.getElementById('mpSoloBtn').onclick = launchSoloGame;
}
...
function launchSoloGame() {
  const name = (document.getElementById('mpPlayerName')?.value || document.getElementById('startName')?.value || 'Jogador').trim().slice(0, 18) || 'Jogador';
  MP.localName = name;
  saveLocalName(name);
  syncNameToGame(name);
...
function connect(onOpen) {
  if (MP.ws && (MP.ws.readyState === WebSocket.OPEN || MP.ws.readyState === WebSocket.CONNECTING)) {
    if (MP.ws.readyState === WebSocket.OPEN) onOpen();
    else MP._onOpen = onOpen;
    return;
  }
  MP.ws = new WebSocket(SERVER_URL);
  MP.ws.onopen = () => { MP.connected = true; const cb = MP._onOpen || onOpen; MP._onOpen = null; cb(); };
  ...
  switch (msg.type) {
    ...
    case 'peer-joined':
      ...
      if (MP.role === 'host') {
        ...
        const startButton = document.getElementById('start');
        if (startButton) startButton.disabled = false;
        setBadge('os dois jogadores conectados — iniciando partida...');
        MP.startGame?.();
      }
      break;
    case 'player-name':
...
function applyHostState(hostState) {
  ...
  if (hostState.started && !hostState.over) document.getElementById('overlay').style.display = 'none';
...
document.addEventListener('DOMContentLoaded', () => {
  const onlineBtn = document.getElementById('onlineMenuBtn');
  const startMenuName = document.getElementById('startName');
  const gameName = document.getElementById('name');
  if (startMenuName && gameName) {
    startMenuName.addEventListener('input', () => {
      gameName.value = startMenuName.value;
      saveLocalName((startMenuName.value || 'Jogador').trim().slice(0, 18) || 'Jogador');
    });
  }
  if (onlineBtn) onlineBtn.addEventListener('click', openOnlineMenu);
});