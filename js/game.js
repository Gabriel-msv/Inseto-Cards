...
// ================================================================
const APP_VERSION = '2.0.0'; // X=reforma, Y=adição, Z=correção de bug.
...
function hasAvailableMove(pi) { let p = state.players[pi]; if (!p || p.moves <= 0) return false; if (p.hand.some(c => c && !CARDS[c.key].type && CARDS[c.key].cost <= p.leaves && ((!p.front) || p.bank.some(x => !x))) ) return true; if (p.front && p.bank.some(x => !x)) return true; return [p.front, ...p.bank].some(c => c && c.hp >= c.maxHp && !CARDS[c.key].type); }
function endTurnIfBlocked(pi) { const p = state.players[pi]; if (validMove(pi) && p.std <= 0 && !hasAvailableMove(pi)) endTurn(pi); }
function startTurn(pi) { if (state.over) return; state.turn = pi; let p = state.players[pi]; p.moves = 1; p.std = 1; if (state.skip[pi]) { state.skip[pi] = false; p.moves = 0; p.std = 0; log(`${p.name} perdeu este turno.`); if (pi === 1) { setTimeout(() => endTurn(1), 700) } return } if (state.round > 1) { p.leaves = Math.min(15, p.leaves + 1) }; for (let x of [p.front, ...p.bank]) if (x) { if (x.root > 0) x.root--; if (x.root === 0) x.effectMarks = (x.effectMarks || []).filter(mark => !['aranha', 'centopeia'].includes(mark)); if (x.skipAttack > 0) x.skipAttack--; if (x.teia > 0) { x.teia--; if (x.teia === 0) { x.skipAttack = 0; x.effectMarks = (x.effectMarks || []).filter(mark => mark !== 'teia'); log(`${CARDS[x.key].name} se libertou da Teia.`) } } if (x.reload > 0) x.reload--; if (x.poisonTurns > 0) { if (x.key !== 'bichoPau') x.hp -= 1; x.poisonTurns--; log(`${CARDS[x.key].name} sofreu 1 de Veneno.`) } else x.poison = 0; if (x.lupa > 0) { if (x.key !== 'bichoPau') x.hp -= 2; x.lupa--; log(`${CARDS[x.key].name} sofreu 2 dano da Lupa.`); if (x.lupa === 0) x.effectMarks = (x.effectMarks || []).filter(mark => mark !== 'lupa'); if (x.hp <= 0) defeat(p, x, 1 - pi) } if (x.activeTurns > 0) { x.activeTurns--; if (x.activeTurns === 0) { let ix = p.bank.indexOf(x); if (ix >= 0) p.bank[ix] = null; if (p.front === x) p.front = null; state.grave.push(x); log(`${CARDS[x.key].name} acabou e foi para o Cemitério.`) } } }; if (p.adubo > 0) p.adubo--; if (p.antiSteal > 0) p.antiSteal--; if (pi === 0) msg('Seu turno. Escolha uma ação.'); else botTurn(); setTimeout(() => endTurnIfBlocked(pi), 900) }
...
function returnToHand(pi, zone, slot) { let p = state.players[pi], c = zone === 'front' ? p.front : p.bank[slot]; FX.boardState('fx-move',480); if (!c) { blockAction('Não há carta nesse espaço.'); return false } if (c.damage > 0 || c.hp < c.maxHp) { blockAction('Só é possível puxar cartas com a vida cheia.'); return false } if (p.moves <= 0) { blockAction('Você não tem movimento disponível.'); return false } if (CARDS[c.key].type === 'effect') { blockAction('Efeitos ativos não podem voltar para a mão.'); return false } if (zone === 'front') p.front = null; else p.bank[slot] = null; p.hand.push(c); p.moves--; log(`${p.name} puxou ${CARDS[c.key].name} de volta para a mão.`); render(); return true }
function onSummon(pi, c) { let p = state.players[pi], o = state.players[1 - pi], d = CARDS[c.key]; if (c.key === 'borboleta') { for (let x of [p.front, ...p.bank]) if (x) { x.hp = x.maxHp; x.damage = 0 } log(`${p.name} curou todos os aliados com a Borboleta.`) }; if (c.key === 'mosca') { let ix = state.grave.findIndex(x => CARDS[x.key].type !== 'effect'); if (ix >= 0) { let old = state.grave.splice(ix, 1)[0]; let nc = card(old.key, pi); nc.atk = Math.max(1, Math.floor(old.atk / 2)); nc.hp = Math.max(1, Math.floor(old.maxHp / 2)); nc.maxHp = nc.hp; p.hand.push(nc) } } if (c.key === 'formigaRainha') drawFree(pi, countField(pi) - 1); if (c.key === 'pulga') steal(pi, 1); if (c.key === 'vaga') { o.revealed = 2; log(`${p.name} revelou 2 cartas da mão do BOT por informação temporária.`) } if (c.key === 'libelula') c.root = 0 }
...
function attack(pi, a, t) {
  ...
  if (a.root > 0 || a.skipAttack > 0 || a.reload > 0) { blockAction(`${CARDS[a.key].name} está impedido de atacar agora.`); return false; }
  if (!t) return false;
...
function resolveTargetEffect(pi, idx, target) {
  ...
  if (o.bank.includes(target) && isBankProtected(1)) { blockAction('O Banco está protegido pelo Própolis.'); return }
  if (c.key === 'inseticida' && !o.bank.includes(target)) { blockAction('Inseticida só atinge o Banco.'); return }
  if (c.key === 'lupa' && ![o.front, ...o.bank].filter(Boolean).includes(target)) { blockAction('Alvo inválido para a Lupa.'); return }
  if (c.key === 'teia' && ![o.front, ...o.bank].filter(Boolean).includes(target)) { blockAction('Alvo inválido para a Teia.'); return }
  if (p.leaves < d.cost) { blockAction('Folhas insuficientes.'); return; }
  ...
  if (c.key === 'lupa' || c.key === 'teia') { c.activeTurns = 3; const slot = p.bank.findIndex(x => !x); if (slot < 0) { p.hand.splice(idx, 0, c); p.leaves += d.cost; blockAction('Você precisa de um espaço vazio no Banco para este efeito.'); return; } p.bank[slot] = c; }
  applyTargetEffect(pi, c, target); render(); winCheck();
}
function applyTargetEffect(pi, c, t) {
  ...
  if (c.key === 'lupa') { t.lupa = 3; t.lupaDamage = 2; t.effectMarks ||= []; if (!t.effectMarks.includes('lupa')) t.effectMarks.push('lupa'); log(`${p.name} colocou Lupa em ${CARDS[t.key].name} por 3 rodadas.`) }
  if (c.key === 'teia') { if (isImmuneRoot(t)) { blockAction('Libélula é imune à Teia.'); return } t.teia = 3; t.skipAttack = 1; t.effectMarks ||= []; if (!t.effectMarks.includes('teia')) t.effectMarks.push('teia'); log(`${p.name} prendeu ${CARDS[t.key].name} com Teia por 3 rodadas.`) }
}
...
render = function() { renderWithActionControls(); renderGrave(); let p = state.players[0]; if (p) { document.getElementById('secondMove').disabled = state.turn !== 0 || state.over || p.std <= 0 || p.moves >= 2; document.getElementById('stdActions').textContent = p.std; document.getElementById('moveActions').textContent = p.moves; document.getElementById('handLeaves').textContent = p.leaves; document.getElementById('handStd').textContent = p.std; document.getElementById('handMoves').textContent = p.moves; syncTouchCardIds(); } };
