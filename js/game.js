...
const CARDS = {
  ...
  propolis: { name: 'Própolis', cost: 3, type: 'effect', emoji: '🍯', cond: 'Ativado', ability: 'Remove veneno dos aliados por 3 rodadas.' },
...
function playEffect(pi, idx) { let p = state.players[pi], c = p.hand[idx], d = CARDS[c.key]; if (p.std <= 0 || p.leaves < d.cost) return false; if (d.equip) { if (!p.front && !p.bank.some(Boolean)) { msg('Não há inseto para equipar.'); return false } state.targetMode = { type: 'equip', pi, idx }; msg('Clique no seu inseto para equipar.'); return false } if (['inseticida', 'lupa', 'teia'].includes(c.key)) { state.targetMode = { type: 'effectTarget', pi, idx }; msg('Clique no alvo inimigo.'); return false } let slot = p.bank.findIndex(x => !x); if (slot < 0) { msg('Não há espaço no Banco para manter o efeito.'); return false } p.leaves -= d.cost; p.hand.splice(idx, 1); c.activeTurns = c.key === 'adubo' ? 4 : 3; p.bank[slot] = c; resolveEffect(pi, c); p.std--; log(`${p.name} ativou ${d.name} no Banco por ${c.activeTurns} rodadas.`); render(); return true }
function resolveEffect(pi, c) { let p = state.players[pi], o = state.players[1 - pi]; switch (c.key) { case 'adubo': p.adubo = 4; break; case 'formigueiro': p.antiSteal = 3; break; case 'propolis': for (let x of [p.front, ...p.bank]) if (x && insect(x)) { x.poison = 0; x.poisonTurns = 0 } break; case 'inseticida': { let t = o.bank.find(Boolean); if (t) { t.hp -= 1; if (t.hp <= 0) defeat(o, t, pi) } break } case 'lupa': { let t = o.front || o.bank.find(Boolean); if (t) { t.hp -= 2; if (t.hp <= 0) defeat(o, t, pi); log('Lupa aplicou 2 dano imediato; o efeito contínuo fica representado pelo estado curto da partida.') } break } case 'teia': { let t = o.front || o.bank.find(Boolean); if (t && !isImmuneRoot(t)) t.skipAttack = 1; break } } }
function equip(pi, idx, target) { let p = state.players[pi], c = p.hand[idx], d = CARDS[c.key]; if (!c || p.leaves < d.cost) return false; p.leaves -= d.cost; p.hand.splice(idx, 1); target.equipment.push(c); if (c.key === 'mel') { target.atk += 1; target.maxHp += 1; target.hp += 1; target.mel = true } if (c.key === 'casulo') { target.maxHp += 2; target.hp += 2 } if (c.key === 'veneno') { target.poison = 1; target.poisonTurns = 2 } p.std--; state.targetMode = null; log(`${p.name} equipou ${d.name} em ${CARDS[target.key].name}.`); render(); return true }
...
function handleDropOnSlot(slotEl, e) {
  ...
  let zone = target === 'pf0' ? 'front' : 'bank'; let slot = zone === 'bank' ? Number(target.slice(-1)) : 0; let destOccupied = zone === 'front' ? !!p.front : !!p.bank[slot]; if (destOccupied) { msg('Esse slot já está ocupado.'); return }
...
function renderSlot(id, c, enemy, front) { let s = document.getElementById(id); s.innerHTML = ''; if (!c) { s.textContent = front ? 'FRONTE' : 'BANCO'; return } let el = makeCard(c, enemy); if (front) el.classList.add('fronte-card'); if (c.equipment && c.equipment.length) { el.classList.add('equipped-card'); c.equipment.forEach(item => { let badge = document.createElement('span'); badge.className = 'equipment-preview'; badge.textContent = CARDS[item.key].emoji; el.appendChild(badge) }) } s.appendChild(el); el.onclick = () => clickCard(enemy ? 1 : 0, front ? 'front' : 'bank', front ? 0 : Number(id.slice(-1))); if (!enemy) el.title = 'Clique para mover'; }
