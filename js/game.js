...
const FX = {
  ...
  boardState(cls, ms=550) {
    ...
    b.classList.remove('fx-attack','fx-hit','fx-summon','fx-harvest','fx-draw','fx-move','fx-discard');
...
function refillNatureFromGrave() {
  if (state.deck.length) return false;
  const dead = state.grave.filter(c => c && c.dead === true && insect(c));
...
function ensureNature() { return state.deck.length > 0 || refillNatureFromGrave(); }
function drawRaw(pi) {
  if (!ensureNature()) return null;
...
function drawCost(pi) {
  ...
  if (!ensureNature()) { if (pi === 0) blockAction('A Natureza está sem cartas e ainda não há cartas derrotadas para reciclar.'); return false; }
...
function drawFree(pi, n) { let got = 0; for (let i = 0; i < n; i++) { if (drawRaw(pi)) got++; } if (got) log(`${state.players[pi].name} recebeu ${got} carta(s) grátis da Natureza.`); }
...
function mandatoryInsectDraw(pi) {
  ...
  let bought = false;
  while (!p.hand.some(insect) && p.leaves >= 3 && ensureNature()) {
...
function hasAvailableStandard(pi) {
  ...
  if ((state.deck.length || state.grave.some(c => c && c.dead === true && insect(c))) && p.leaves >= 3) return true;
  if (p.front) {
    const a = p.front;
...
function clearSoloTurnTimer() {
  clearTimeout(soloTurnTimer); clearInterval(soloTurnTicker);
  soloTurnTimer = null; soloTurnTicker = null;
  state.mpTurnDeadline = 0;
  const t = document.getElementById('mpTurnTimer'); if (t) t.hidden = true;
  const f = document.getElementById('mpFixedTimer'); if (f && !window.MP?.active) f.hidden = true;
}
function updateSoloTurnTimer() {
  if (window.MP?.active || !state.started || state.over || state.turn !== 0) return;
  const text = formatTurnTime(state.mpTurnDeadline);
  const t = document.getElementById('mpTurnTimer'); if (t) { t.hidden = false; t.textContent = text; }
  let f = document.getElementById('mpFixedTimer');
  if (!f) { f = document.createElement('div'); f.id = 'mpFixedTimer'; document.body.appendChild(f); }
  f.hidden = false; f.textContent = text;
  f.classList.toggle('urgent', Math.max(0, state.mpTurnDeadline - Date.now()) <= 5000);
}
function armSoloTurnTimer(pi) {
  clearSoloTurnTimer();
  if (window.MP?.active || pi !== 0 || !state.started || state.over) return;
  state.mpTurnDeadline = Date.now() + SOLO_TURN_LIMIT;
  updateSoloTurnTimer();
  soloTurnTicker = setInterval(updateSoloTurnTimer, 250);
  soloTurnTimer = setTimeout(() => {
    if (!window.MP?.active && state.started && !state.over && state.turn === 0) {
      log(`${state.players[0].name} perdeu o turno por não agir em 30 segundos.`);
      endTurn(0);
    }
  }, SOLO_TURN_LIMIT);
}
...
function endTurn(pi) { if (state.over) return; clearSoloTurnTimer(); if (pi === 0) startTurn(1); else { state.round++; startTurn(0) }; render(); winCheck() }
// ---------------------------------------------------------------
...
function onSummon(pi, c) {
  ...
  if (c.key === 'vaga') {
    o.revealed = 2;
    log(`${p.name} revelou 2 cartas da mão do BOT por informação temporária.`);
  }
...
function attack(pi, a, t) {
  ...
  if (!a) { if (pi === 0) blockAction('Não há inseto no Fronte para atacar.'); return false; }
  if (a.root > 0) { if (pi === 0) blockAction(`${CARDS[a.key].name} está presa e não pode se mover ou atacar agora.`); return false; }
  if (a.skipAttack > 0) { if (pi === 0) blockAction(`${CARDS[a.key].name} não pode atacar neste turno.`); return false; }
  if (a.reload > 0) { if (pi === 0) blockAction(`${CARDS[a.key].name} está recarregando e ainda não pode atacar.`); return false; }
  if (!t) { if (pi === 0) blockAction('Não há alvo válido para atacar.'); return false; }
  let targetOwner = t === o.front ? 1 - pi : o.bank.includes(t) ? 1 - pi : -1;
  if (targetOwner >= 0 && o.bank.includes(t) && isBankProtected(targetOwner)) { if (pi === 0) blockAction('O Banco está protegido pelo Própolis.'); return false }
  let d = CARDS[a.key];
  p.std--;
  ...
  if (a.key === 'cupim') { const hadBuff = clearBuffs(t); showCupimModal(t, hadBuff); }
...
function directAttack(pi) {
  ...
  log(`${state.players[pi].name} fez um ataque direto e descartou uma carta da mão inimiga.`);
  if (pi === 1) showEnemyActions(['Ataque direto: uma carta da mão foi descartada.'], 'ATAQUE DIRETO DO BOT');
  else showEnemyActions(['Você descartou uma carta da mão inimiga.'], 'ATAQUE DIRETO');
  return true;
}
...
function resolveTargetEffect(pi, idx, target) {
  ...
  const enemies = fieldCards(1-pi);
  if (!target || !enemies.includes(target)) { blockAction(`Alvo inválido para ${d.name}.`); return false; }
  ...
  p.leaves -= d.cost; p.hand.splice(idx, 1); state.targetMode = null; p.std--;
  if (c.key === 'inseticida') {
    target.hp -= 1;
    ...
    c.activeTurns = 3; p.bank[bankSlot] = c; target.lupa = 3; target.effectMarks ||= []; if (!target.effectMarks.includes('lupa')) target.effectMarks.push('lupa');
    log(`${p.name} colocou Lupa em ${CARDS[target.key].name} por 3 rodadas.`);
  } else if (c.key === 'teia') {
    c.activeTurns = 1; p.bank[bankSlot] = c; target.teia = 1; target.skipAttack = 1; target.effectMarks ||= []; if (!target.effectMarks.includes('teia')) target.effectMarks.push('teia');
    log(`${p.name} prendeu ${CARDS[target.key].name} com Teia para o próximo turno.`);
  }
  render(); winCheck(); endTurnIfBlocked(pi); return true;
}
...
function sellSelected() {
  ...
  if (state.selected !== null) {
    ...
    p.hand.splice(state.selected, 1); c.dead = false; state.grave.push(c); FX.boardState('fx-discard', 500);
  ...
  c.dead = false; state.grave.push(c); FX.boardState('fx-discard', 500); p.leaves = Math.min(15, p.leaves + 1); p.std--; state.selectedField = null;
  log(`${p.name} vendeu ${CARDS[c.key].name} por 1 folha.`);
...
function playerDraw() {
  ...
  if (!ensureNature()) { blockAction('A Natureza está sem cartas e ainda não há cartas derrotadas para reciclar.'); return; }
...
function handleDropOnSlot(slotEl, e) {
  ...
  if (enemyTarget) {
    if (found.zone !== 'front' || p.std <= 0) { blockAction('Arraste apenas o inseto do seu Fronte para atacar.'); return; }
    const o = state.players[1];
    let targetCard = null;
    if (target === 'ef0') targetCard = o.front;
    else if (target.startsWith('eb')) targetCard = o.bank[Number(target.slice(-1))];
    if (!targetCard) { blockAction('Solte a carta exatamente sobre um alvo do Banco.'); return; }
    if (!isValidAttackTarget(found.c, targetCard)) { blockAction('Esse alvo não é válido para este ataque.'); return; }
...
// ================================================================
function render() { refreshPassiveStats(); let p = state.players[0], o = state.players[1]; if (!p) return; document.getElementById('round').textContent = state.round; document.getElementById('turn').textContent = state.turn === 0 ? 'VOCÊ' : 'BOT'; document.getElementById('turnNo').textContent = state.round; document.getElementById('deckCount').textContent = state.deck.length; document.getElementById('deckCount2').textContent = state.deck.length; document.getElementById('graveCount').textContent = state.grave.length; document.getElementById('graveCount2').textContent = state.grave.length; document.getElementById('playerLeaves').textContent = p.leaves; document.getElementById('enemyLeaves').textContent = o.leaves; document.getElementById('playerName').textContent = p.name.toUpperCase(); document.getElementById('moves').textContent = p.moves; document.getElementById('std').textContent = p.std; document.getElementById('draw').disabled = state.turn !== 0 || p.std <= 0 || p.leaves < 3 || (!state.deck.length && !state.grave.some(c => c && c.dead === true && insect(c))); document.getElementById('harvest').disabled = state.turn !== 0 || p.std <= 0; document.getElementById('attackBtn').disabled = state.turn !== 0 || state.over || p.std <= 0 || !p.front; document.getElementById('secondMove').disabled = state.turn !== 0 || state.over || p.moves <= 0 || p.std <= 0; document.getElementById('returnBtn').disabled = state.turn !== 0 || state.over || p.moves <= 0 || !state.selectedField; document.getElementById('sell').disabled = state.turn !== 0 || p.std <= 0 || state.selected === null && !state.selectedField; document.getElementById('end').disabled = state.turn !== 0 || state.over; document.getElementById('attackBtn').classList.toggle('waiting', !!(state.targetMode && state.targetMode.type === 'attack')); document.getElementById('returnBtn').classList.toggle('waiting', !!state.selectedField); document.getElementById('sell').classList.toggle('waiting', !!(state.selected !== null || state.selectedField)); renderSlot('ef0', o.front, true, true); for (let i = 0; i < 3; i++) { renderSlot('eb' + i, o.bank[i], true, false); renderSlot('pb' + i, p.bank[i], false, false) } renderSlot('pf0', p.front, false, true); let h = document.getElementById('hand'); h.innerHTML = ''; p.hand.forEach((c, i) => { let el = makeCard(c, false); if (state.selected === i) el.classList.add('selected'); el.onclick = () => clickCard(0, 'hand', i); h.appendChild(el) }); const logEl = document.getElementById('log'); logEl.innerHTML = state.log.map(x => `<div>› ${x}</div>`).join(''); logEl.scrollTop = logEl.scrollHeight }
...
function showTip(e, c) {
  const d = CARDS[c.key], t = document.getElementById('tip');
  if (!t || !d) return;
  clearTimeout(tipHideTimer);
  document.getElementById('tipName').textContent = d.name;
  document.getElementById('tipCond').textContent = '[' + d.cond + ']';
  document.getElementById('tipAbility').textContent = d.ability + (c.customAbility ? ' | Copiada: ' + c.customAbility : '');
  t.style.display = 'block';
  t.style.left = Math.min(innerWidth - 255, e.clientX + 10) + 'px';
  t.style.top = Math.min(innerHeight - 150, e.clientY + 10) + 'px';
  tipHideTimer = setTimeout(hideTip, 3200);
}
function hideTip() {
  clearTimeout(tipHideTimer);
...
function showCupimModal(target, hadBuff) {
  const targetArt = CARD_IMAGES[target.key];
  const equip = (target.equipment || []).filter(Boolean).slice(-2);
  const cards = equip.length ? equip : [target];
  const slots = [cards[0], cards[1] || null];
  const cardHtml = slots.map((c, i) => c ? `<div class="cupim-preview-card"><img src="${CARD_IMAGES[c.key] || ''}" alt="${CARDS[c.key].name}"><span>${CARDS[c.key].name}</span></div>` : `<div class="cupim-preview-card empty"><span>${hadBuff ? 'Sem outro buff' : 'Sem buff adicional'}</span></div>`).join('');
  showModal(`<div class="cupim-modal"><div class="cupim-kicker">CUPIM</div><h2>BUFFS ANULADOS</h2><p>Alvo: <b>${CARDS[target.key].name}</b></p><div class="cupim-preview-grid">${cardHtml}</div><p class="small">O efeito do Cupim apenas remove os buffs do alvo; esta visualização mostra o alvo e os efeitos de equipamento encontrados.</p></div>`);
  setTimeout(() => { const o = document.getElementById('overlay'); if (o && o.style.display === 'flex') o.style.display = 'none'; }, 2600);
}
function showModal(html) { document.getElementById('modalContent').innerHTML = html; document.getElementById('overlay').style.display = 'flex' } function rules() { showModal(`<h2>INSETO CARDS — REGRAS</h2><ul><li>2 jogadores; cada um tem 1 Fronte, 3 Banco e mão própria. Natureza e Cemitério são compartilhados.</li><li>Cada turno: 1 Movimento + 1 Ação Padrão. Você pode abrir mão da Padrão para ganhar um segundo Movimento.</li><li>Movimento: invocar carta, mover campo ou devolver carta sem dano à mão.</li><li>Padrão: atacar, colher +1, vender, ou comprar da Natureza por 3 folhas.</li><li>Começo: 5 folhas e 3 cartas. A partir da rodada 2: +1 folha automática. Limite 15.</li><li>Combate é mútuo. O Fronte só pode atacar Banco se o Fronte inimigo estiver vazio, exceto Meganeura.</li><li>Vitória: eliminar todos os insetos da mão e do campo inimigo; efeitos não contam.</li></ul><h3 style="color:var(--amber);margin:10px 0 5px">Decisões necessárias para a implementação</h3><p>Venda = +1 folha; Vaga-lume revela 2 cartas da mão do inimigo apenas no log; empate técnico = se ambos zerarem na mesma resolução, a partida termina empatada. A mão inicial foi fixada em 3, conforme as simulações mencionadas no GDD.</p><p class="small" style="margin-top:10px">O baralho usa as 44 cartas do catálogo atual, uma cópia de cada.</p>`) }
...
document.addEventListener('pointerdown', e => { if (!e.target.closest('.card, #tip')) hideTip(); });
...
render = function() {
  ...
  if (p) {
    ...
    syncTouchCardIds();
    if (state.turn === 0 && !state.over) setTimeout(() => endTurnIfBlocked(0), 0);
  }
};
